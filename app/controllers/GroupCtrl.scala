...
object GroupCtrl extends Controller {
  def createGroup() = Action.async {
    request => {
      ...
      val group = Group.createGroup(uid, name, groupType, isPublic)
      group.map(msg => {
        val result = JsObject(Seq(
          "groupId" -> JsNumber(msg.getGroupId.toLong),
          "name" -> JsString(msg.getName),
          "creator" -> JsNumber(msg.getCreator.toLong),
          "groupType" -> JsString(msg.getType),
          "isPublic" -> JsBoolean(msg.getVisible)
        ))
